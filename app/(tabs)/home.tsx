...
export default function HomeScreen() {
  ...
  const stats = [
    {
      label: "Distance",
      value: `${((dashboard?.metrics.distanceMeters ?? 0) / 1000).toFixed(1)}`,
      unit: "km",
    },
    {
      label: "Trips today",
      value: String(dashboard?.metrics.tripCount ?? 0),
      unit: "drives",
    },
    {
      label: "Reports",
      value: String(dashboard?.metrics.reportCount ?? 0),
      unit: "shared",
    },
  ];
    ...
    >
      ...
      <View style={styles.tripCard}>
        <View style={styles.tripTopRow}>
          <View style={styles.statusPill}>
            <View
              style={[
                styles.statusDot,
                { backgroundColor: trackingActive ? "#74E7B7" : "#CDBEFF" },
              ]}
            />
            <Text style={styles.statusText}>
              {trackingActive ? "TRACKING ACTIVE" : "READY TO TRACK"}
            </Text>
          </View>
          <MaterialCommunityIcons
            color="#CDBEFF"
        ...
        <Text style={styles.tripDescription}>
          {trackingActive && dashboard?.metrics.currentSpeedMps != null
            ? `Current speed: ${Math.round(dashboard.metrics.currentSpeedMps * 3.6)} km/h`
            : "Share anonymous movement data to improve traffic across Accra."}
        </Text>
        <Pressable
          accessibilityRole="button"
        ...
        >
          ...
          <Text style={styles.startButtonText}>
            {trackingActive ? "View active drive" : "Start a drive"}
          </Text>
          <MaterialCommunityIcons
            color="#5B21F0"
            name="arrow-right"
      ...
      {error && (
        <Pressable
          ...
          style={[styles.errorRow, { backgroundColor: theme.dangerSoft }]}
        >
          <MaterialCommunityIcons
            color={theme.danger}
      ...
      <View style={styles.sectionHeading}>
        <View>
          ...
          >
            Your driving tools, all in one place.
          </Text>
        </View>
      </View>
        ...
        {quickActions.map((action) => (
          ...
          >
            ...
            <View style={styles.actionCopy}>
              ...
              <Text
                style={[styles.actionDetail, { color: theme.textSecondary }]}
              >
                {action.detail}
              </Text>
            </View>
      ...
      >
        {dashboard?.incidents.map((incident, index) => (
          ...
          >
            ...
            <View style={styles.incidentCopy}>
              <Text
                numberOfLines={1}
                style={[styles.incidentTitle, { color: theme.textPrimary }]}
              >
                {incident.type}
              </Text>
              <Text
                numberOfLines={1}
                style={[styles.incidentRoad, { color: theme.textSecondary }]}
              >
                {incident.roadName}, {incident.city}
              </Text>
            </View>
            <View style={styles.incidentTimeWrap}>
              <Text
                style={[styles.incidentTime, { color: theme.textSecondary }]}
              >
        ...
        {!loading && !error && dashboard?.incidents.length === 0 && (
