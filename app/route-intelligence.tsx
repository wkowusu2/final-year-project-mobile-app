...
function RouteCard({ route, recommended, selected, onPress }: { route: RouteAlternative; recommended: boolean; selected: boolean; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`View ${recommended ? 'recommended' : 'alternative'} route on map`} onPress={onPress}>
    ...
    {route.advisories.length > 0 && <View style={[styles.incidentNotice, { borderColor: theme.danger, backgroundColor: theme.dangerSoft }]}><MaterialCommunityIcons name="road-variant" color={theme.danger} size={17} /><Text style={[styles.incidentText, { color: theme.textPrimary }]}>{route.advisories.length} active road work{route.advisories.length > 1 ? 's' : ''} or advisory on this route</Text></View>}
