...
function IncidentContent({ incident, confirming, onConfirm }: { incident: IncidentDetail; confirming: boolean; onConfirm: () => void }) {
  ...
  const accent = danger ? theme.danger : incident.severity === 'medium' ? theme.warning : theme.success;
  const tint = danger ? theme.dangerSoft : incident.severity === 'medium' ? theme.warningSoft : theme.successSoft;
  return <>
    <View style={[styles.hero, { backgroundColor: tint, borderColor: accent }]}><View style={[styles.heroIcon, { backgroundColor: theme.surfaceSecondary }]}><MaterialCommunityIcons color={accent} name="alert-outline" size={25} /></View><View style={styles.heroCopy}><Text style={[styles.eyebrow, { color: accent }]}>ROAD INCIDENT</Text><Text style={[styles.title, { color: theme.textPrimary }]}>{incident.type}</Text><Text style={[styles.time, { color: theme.textSecondary }]}>{relativeTime(incident.createdAt)}</Text></View><View style={[styles.statusPill, { backgroundColor: theme.surfaceSecondary }]}><Text style={[styles.status, { color: accent }]}>{incident.status.toUpperCase()}</Text></View></View>
